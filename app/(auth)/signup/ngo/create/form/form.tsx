...
export default function Form() {
  ...
  const { data, setData, ngo_signup, ngo_signup_verify_transaction } =
    useSignupContext();
  ...
  }, [data?.password, confirmPassword, termsAndConditionsAgreement]);

  useEffect(() => {
    if (transactionReference) {
      ngo_signup_verify_transaction(setSigningUp, transactionReference);
    }
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center gap-[5px] lg:gap-[10px] leading-tight text-[14px] lg:text-[16px] mt-[20px]">
        <div
          className={`w-[12px] h-[12px] lg:w-[17px] lg:h-[17px] rounded-full ${
            data?.password?.length >= 6 && "bg-[#026935]"
          } border border-[#026935] text-white flex items-center justify-center text-base mb-[2px]`}
        >
          {data?.password?.length >= 6 && (
            <IoIosCheckmark className="lg:text-[20px]" />
          )}
      ...
      <div className="flex items-center gap-[5px] lg:gap-[10px] leading-tight text-[14px] lg:text-[16px] mt-[15px]">
        <div
          className={`w-[12px] h-[12px] lg:w-[17px] lg:h-[17px] rounded-full ${
            data?.password === confirmPassword && "bg-[#026935]"
          } border border-[#026935] text-white flex items-center justify-center text-base mb-[2px]`}
        >
          {data?.password === confirmPassword && (
            <IoIosCheckmark className="lg:text-[20px]" />
      ...
      <div className="mt-[20px] flex flex-col lg:flex-row gap-[10px] lg:gap-[25px]">
        <div
          onClick={() => router.push("/signup/ngo/ngo-info")}
        ...
        <button
          ...
          onClick={() => ngo_signup(setSigningUp)}
          className={`cursor-pointer py-[10px] px-[33px] text-white rounded-[10px] font-semibold w-max ${
            check ? "bg-[#026935]" : "bg-gray-600"
          }`}
        >
          {signingUp ? "Signing Up..." : "Register as an NGO"}
        </button>
      </div>
      {signingUp && (
        <div className="fixed top-0 right-0 w-full h-full bg-[#0000004d] z-50 flex items-center justify-center">
