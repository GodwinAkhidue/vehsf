...
export default function Skills({
  ...
  const add_new_skill = (name: any) => {
    setSelectedSkills((prev: any) => [...prev, name]);
  ...
  useEffect(() => {
    if (skills && skills.length > 0) {
      ...
      const filter = skills.filter((i: any) =>
        i.name.toLowerCase().includes(skillSearchValue.toLowerCase())
      );
    ...
    <div className="mt-[30px] lg:mt-[40px] relative">
      ...
      {selectedSkills && selectedSkills.length > 0 && (
        <div className="mt-[20px] flex flex-wrap w-full gap-x-[20px] gap-y-[10px]">
          {selectedSkills.map((i: any, index: any) => (
            <Selected_Skill_Item
              name={i}
              key={index}
              delete_skill={(name: any) =>
                setSelectedSkills((prev: any) => {
                  return prev.filter((i: any) => i !== name);
                })
              }
            />
          ))}
      ...
      <div
        ...
        className={`w-full absolute bg-white top-[95px] ${
          displaySkillList ? "block" : "hidden"
        } h-max min-h-[50px] max-h-[300px] overflow-y-auto border border-[#00000080] z-10 rounded-[7px]`}
      >
        {isFetchingSkills ? (
          ...
          <div className="h-max">
            {filteredSkills ? (
              ...
              filteredSkills.map((i: any, index: any) => (
                <div
                  key={index}
